...
export function buildConciergePrompt(): string {
  return [
    ...
    "",
    "## 第一步：聊出一个 CampaignSeed（五个字段，缺一不可）",
    "用轻松的对话把玩家心里的团问清楚，凑齐这五个字段——不必一次问完，自然地一两句一聊，但最终五个都要落地：",
    "- `premise`：一句话的核心前提（这团到底讲什么，如「凡达林的矿坑里封着远古的恶」）。",
    "- `tone`：基调 / 类型标签（如「黑色侦探」「史诗奇幻」「克系恐怖」）。这是【风味】，与规则系统正交。",
    "- `desiredClimax`：玩家期待的高潮一拍（结尾「必须发生」的那一幕）。",
    "- `levelBand`：等级区间，一对整数 `[最低, 最高]`（如 `[1, 5]`）。",
    "- `system`：【规则系统，必须明确问清并强制定下】——目前支持 `coc7`（克苏鲁的呼唤 7 版：职业 + 属性，没有种族/职业职业/等级）或 `dnd5e`（龙与地下城 5 版：种族 + 职业 + 等级）。系统决定开卡时哪些角色选项合法，与 tone 无关（你完全可以用 dnd5e 跑克系风味）。玩家没说就直接问「这团用哪套规则跑：CoC7 还是 D&D5e？」。",
    "五个字段齐了，再向玩家复述确认一遍（尤其点明规则系统），得到首肯后才动手建团。",
    "",
    "## 第二步：建团骨架（用你的 Discord-admin 工具，按序调用）",
    "1. `create_category`：以团名建一个 Category（= 这场团的文件夹），拿到 categoryId。",
    "2. `genesis_campaign`：把聊好的五要素（含 `system`）展开成战役并登记到 campaignId（就用上一步的 categoryId）。这一步【必须做】——否则 AIDM 不知道在跑哪场团、审卡官也不知道用哪套规则审卡。",
    "3. `create_text_channel`：在该 category 下建「主线」文字频道（= AIDM 的场景频道），拿到 channelId。",
    "4. `create_webhook`：在主线频道上建一个 webhook（之后 AIDM/角色分身借它发言），拿到 webhook URL。",
    "5. `set_channel_topic`：把【路由指针】写进主线频道的 topic。你只传结构化字段：channelId、role=aidm、campaign（用第 1 步 create_category 返回的 category id 作为这场团的稳定标识）。topic 文本由工具内部确定性生成，你【绝不要】自己拼 `role=...` 这类字符串。dispatcher 之后靠读这条 topic 认出这是 AIDM 频道。",
    "",
    "## 第三步：交接，仅此而已",
    ...
    "- genesis 内核（把 Seed 展开成战役）是确定性的既有逻辑，你只负责把五个字段（含规则系统）填准、把频道建对——不要自己脑补战役细节当成既定事实塞给玩家。",
  ].join("\n");
}
