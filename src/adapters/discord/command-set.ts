...
export const COMMAND_OPTIONS: Readonly<Record<string, readonly CommandOption[]>> = {
  ...
  // `/check`: optional STRING `skill` (#54 — name a check to REQUEST when none is
  // pending; the DM must answer it) + optional STRING `advantage` (优势/劣势 →
  // advantage/disadvantage; anything else → a straight roll, parsed leniently).
  check: [
    { type: 3, name: "skill", description: "你要的检定技能，如 侦查（有待掷检定时可留空）" },
